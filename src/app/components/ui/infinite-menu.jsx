...
// Clases de geometría y utilidades
class Face {
  constructor(a, b, c) {
    this.a = a;
    this.b = b;
...
class ArcballControl {
  ...
  // Resto de los métodos de ArcballControl del ejemplo original...
  ...
  // Resto de los métodos de ArcballControl...
...
class InfiniteGridMenu {
  ...
  #onControlUpdate(deltaTime) {
    ...
    if (!this.control.isPointerDown) {
      ...
      const previewVertexIndex = this.#findPreviewVertexIndex();
      const previewItemIndex = previewVertexIndex % Math.max(1, this.items.length);
      
      if (this.onPreviewItemChange) {
        this.onPreviewItemChange(previewItemIndex);
      }
  ...
  #findPreviewVertexIndex() {
    ...
    let maxD = -1;
    let nearestVertexIndex;
    const rotationDirection = this.control.rotationVelocity > 0 ? 1 : -1;
    ...
    const itemCount = Math.max(1, this.items.length);
    const currentIndex = nearestVertexIndex % itemCount;
    const previewIndex = (currentIndex + rotationDirection + itemCount) % itemCount;
    
    return this.instancePositions.findIndex((pos, index) => 
      index % itemCount === previewIndex
    );
  }
}
...
export default function InfiniteMenu({ 
  ...
  useEffect(() => {
    ...
    const handlePreviewItem = (index) => {
      const itemIndex = index % items.length;
      setPreviewItem(items[itemIndex]);
    };
    ...
    if (canvas) {
      sketch = new InfiniteGridMenu(
        ...
        setIsMoving,
        (sk) => {
          sk.onPreviewItemChange = handlePreviewItem;
          sk.run();
        }
      );
    }
  ...
  const currentItem = isMoving ? previewItem : activeItem;
        ...
        <>
          <div 
            className={`
              ...
              max-w-[400px]
              ${isMoving
                ? "opacity-70 pointer-events-none duration-[100ms] translate-x-[-20%]"
          ...
          >
            ...
            {showDetails && currentItem.ofertaData && (
              <div className="bg-white/5 rounded-xl p-4 space-y-2">
                <div className="flex justify-between">
                  <span className="text-white/70">Modalidad:</span>
                  <span className="text-white font-semibold">
                    {currentItem.ofertaData.modalidad || 'No especificada'}
                  </span>
                </div>
                <div className="flex justify-between">
                  <span className="text-white/70">Duración:</span>
                  <span className="text-white font-semibold">
                    {currentItem.ofertaData.duracion || 'No especificada'}
                  </span>
                </div>
                <div className="flex justify-between">
                  <span className="text-white/70">Requisitos:</span>
                  <span className="text-white font-semibold">
                    {currentItem.ofertaData.requisitos || 'No especificados'}
                  </span>
                </div>
              </div>
            )}
          ...
          <div className="absolute top-[1.6em] right-[1.6em] flex space-x-2">
            {showListButton && (
              <button 
                onClick={() => onListView && onListView()}
                className="
                  bg-white/10 
                  hover:bg-white/20 
                  text-white 
