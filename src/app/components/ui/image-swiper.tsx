...
interface ImageSwiperProps {
  ...
  cardHeight?: number;
  className?: string;
  children?: ((currentIndex: number) => ReactNode) | ReactNode;
}

export const ImageSwiper: React.FC<ImageSwiperProps> = ({
  images,
  cardWidth = 300,  // Ajustado para coincidir con OfertaCard
  cardHeight = 400, // Ajustado para coincidir con OfertaCard
  className = '',
  children
}) => {
  ...
  const renderCardContent = (originalIndex: number) => {
    if (typeof children === 'function') {
      return children(originalIndex);
    }
    return (
      <img
  ...
  return (
    ...
    >
      {cardOrder.map((originalIndex, displayIndex) => (
        ...
        >
          {renderCardContent(originalIndex)}
        </article>
      ))}
    </section>
  );
